...
let settingsRows = computed(() => {
    ...
    let rememberedRows =
        store.getters["SettingsStore/getChainPermissions"](chain);
...
ipcRenderer.on("rawdeeplink", async (event, args) => {
    ...
    if (
        ...
        router.currentRoute.value.path != "/raw-link"
    ) {
    ...
    if (settingsRows.value && !settingsRows.value.includes(apiobj.type)) {
    ...
    if (apiobj.type === Actions.INJECTED_CALL) {
        ...
        for (let i = 0; i < tr.operations.length; i++) {
            ...
            if (
                settingsRows.value &&
                settingsRows.value.includes(operation[0])
            ) {
                authorizedUse = true;
                break;
            }
        ...
        if (!authorizedUse) {
            console.log(
                `Unauthorized use of raw deeplinked ${chain} blockchain operation`
            );
            deepLinkInProgress.value = false;
            return;
        }
        ...
        <span v-if="supportsTOTP">
            ...
            <span v-else>
                ...
                <ui-card v-shadow="3" outlined style="margintop: 5px">
                    ...
                    <span
                        v-else-if="
                            opPermissions == 'Configure' && !selectedRows
                        "
                    >
                        <Operations />
                    </span>
                    ...
                    <span v-if="opPermissions && settingsRows && selectedRows">
                        <ui-chips id="input-chip-set" type="input">
                            <ui-chip icon="security" style="margin-left: 30px">
                                ...
                                {{ t("common.totp.chosen") }}
                            </ui-chip>
                            <ui-chip icon="thumb_up">
                                Ready for raw links!
                            </ui-chip>
                        </ui-chips>
                    </span>
