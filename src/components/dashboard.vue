...
export default {
  ...
  methods: {
    ...
    requestVote: async function(request) {
      ...
      let entity='';
      let objdata=await this.$root.$data.api.db_api()
      ...
      switch (entity_id[1]) {
        case '5':
          ...
          objextradata=await this.$root.$data.api.db_api()
        .exec("get_objects", [[objdata[0].committee_member_account]]);
          this.$data.specifics="Commitee member: "+objextradata[0].name+"\nCommittee Member ID: "+this.$data.incoming.id;
        ...
        .exec("get_objects", [[objdata[0].witness_account]]);
          this.$data.specifics="Witness: "+objextradata[0].name+"\nWitness ID: "+this.$data.incoming.id;
          this.$data.incoming.vote_id=objdata[0].vote_id;
          break;
        case '14':
        ...
        .exec("get_objects", [[objdata[0].worker_account]]);
         this.$data.specifics="Proposal: "+objdata[0].name+"\nProposal ID: "+this.$data.incoming.id+"\nDaily Pay: "+this.formatMoney(objdata[0].daily_pay/Math.pow(10,5),5)+"BTS\nWorker: "+objextradata[0].name;
    ...
    acceptGeneric: async function() {
      ...
      let res = await this.$root.$data.api.init_promise;
        let operation=await Operations.generate(this.$data.incoming, this.$root.$data.api,this.$root.$data.wallet.accountID);
