...
export default class Steem extends BlockchainAPI {
    ...
    connect(nodeToConnect, onClose = null) {
        return new Promise((resolve, reject) => {
            ...
            //smokejs.api.setOptions({ url: nodeToConnect });
            resolve();
        });
    }
    ...
    getAccount(accountname) {
        return new Promise((resolve, reject) => {
            smokejs.api.getAccounts([accountname], function(err, result) {
                if (result.length == 0) {
    ...
    getPublicKey(privateKey) {
        return smokejs.auth.wifToPublic(privateKey);
    }
            ...
            this._ensureAPI().then(() => {
                if (!!transaction.type) {
                    ...
                    if (typeof transaction == "object"
                        ...
                        smokejs.broadcast[operationName](
                            ...transaction,
                            (err, result) => {
